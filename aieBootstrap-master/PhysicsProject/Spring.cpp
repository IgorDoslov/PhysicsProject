...
Spring::Spring(Rigidbody* a_actor1, Rigidbody* a_actor2, float a_dampening, 
	float a_springCoefficient, float a_restLength, glm::vec2 a_contact1, glm::vec2 a_contact2) : PhysicsObject(JOINT),
	m_actor1(a_actor1), m_actor2(a_actor2), m_dampening(a_dampening), m_springCoefficient(a_springCoefficient),
...
void Spring::FixedUpdate(glm::vec2 a_gravity, float a_timeStep)
{
	...
	// F = -kx - bv
	glm::vec2 force = direction * m_springCoefficient * (m_restLength - length) - m_dampening * relativeVelocity;

	// We will cap the spring force 1000N to prevent numerical instability
